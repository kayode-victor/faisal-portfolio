...
export default function Wedding() {
  ...
  const openModal = (index) => {
    ...
    setTempSrc(weddingData[index].src);
    setModel(true);
  };
  const closeModal = () => {
    setModel(false);
  ...
  const nextImage = () => {
    const nextIndex = (currentIndex + 1) % weddingData.length;
    setCurrentIndex(nextIndex);
    setTempSrc(weddingData[nextIndex].src);
  };

  const prevImage = () => {
    const prevIndex =
      (currentIndex - 1 + weddingData.length) % weddingData.length;
    setCurrentIndex(prevIndex);
    setTempSrc(weddingData[prevIndex].src);
  };
